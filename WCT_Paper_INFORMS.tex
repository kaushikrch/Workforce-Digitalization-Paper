...
We examine the model's predictions using three complementary identification strategies at different levels of aggregation: (i)~firm-level cross-sectional and event-study evidence using customer satisfaction and Glassdoor data ($N = 131$--$216$, 13--17 firms); (ii)~county-level panel evidence using Bureau of Labor Statistics (BLS) retail employment and wages ($N = 30{,}951$, 3{,}115 US counties). The firm-level analysis tests the Front-Stage Trap (inverted-U in e-commerce) and complementarity (technology--workforce interaction). The county-level analysis tests the Good Jobs Buffer at a different margin: whether counties with higher initial retail wages show more resilient retail employment as e-commerce penetration grows. An independent European sample ($N_c = 11$, $N = 20$) is reported in the EC (\S EC.5) for transparency.
...
ACSI score & 131 & 76.3 & 5.2 & 62--87 \\
Glassdoor rating (1--5) & 131 & 3.29 & 0.44 & 2.5--4.3 \\
CapEx / Revenue (\%) & 131 & 3.8 & 1.9 & 1.0--9.2 \\
\addlinespace
...
\noindent\textit{Cross-sectional complementarity (Proposition~\ref{prop:complementarity}).} The between-effects (OLS) estimate of the standardised interaction $K^z \times W^z$ is positive ($+1.95$), consistent with the prediction that firms with jointly higher technology and workforce investment enjoy higher satisfaction. A one-standard-deviation joint increase in $K$ and $W$ is associated with a 1.95-point ACSI gain. The direction aligns with the complementarity evidence in \citet{aral2012three}. Within-firm estimates (firm FE) are negative, as expected given the low within-firm Glassdoor SD (30\% of total SD); the between-effects estimate is the appropriate estimand for a structural cross-firm relationship \citep{milgrom1990economics}.

\noindent\textit{Inverted-U (Theorem~\ref{thm:trap}).} The quadratic specification yields $\hat{\beta}_2 = -0.028$ ($p = 0.028$ under $t(16)$), consistent with the predicted inverted-U. The turning point (7.9\% e-commerce share) corresponds to the mid-2010s transition, broadly consistent with $F_{\mathrm{crit}}$. The coefficient attenuates when workforce controls are included ($p = 0.31$), consistent with the model's prediction that e-commerce damages satisfaction \emph{through} workforce depletion.
...
\textit{Robustness.} The result strengthens when COVID years are excluded ($\hat{\beta}_1 = 0.0031$, $p = 0.014$), holds in urban counties ($p = 0.006$), and is insignificant in rural counties ($p = 0.58$), consistent with the mechanism operating where retail employment is concentrated. A pre-period placebo (2015--2017, when e-commerce grew modestly) shows no effect ($p = 0.40$), confirming that the wage buffer activates only under substantial digital pressure. The effect operates through employment quantity, not wage levels: using wage growth as the dependent variable yields a null result ($p = 0.98$), consistent with the model's capacity-multiplier channel.

\subsection{Assessment}\label{sec:emp:assessment}

Three findings across two levels of analysis support the model's qualitative predictions. First, the inverted-U between e-commerce and ACSI (Pattern~2, $p = 0.028$) and the post-shock ACSI decline in the event study ($p = 0.037$) both support the Front-Stage Trap. Second, the county-level wage--e-commerce interaction ($p = 0.025$, $N = 30{,}951$) provides a well-powered test of the Good Jobs Buffer at the employment-capacity margin. Third, the between-effects complementarity ($+1.95$) is directionally consistent but imprecisely estimated. The empirical contribution is one of convergent evidence across different identification strategies, levels of analysis, and outcome variables, subject to the limitations in \S\ref{sec:disc:limits}.
