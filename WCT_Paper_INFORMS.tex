...
Three extensions, in order of expected impact: (i)~identification of $p$ from retailer panel data, the model's central testable prediction (the interaction coefficient in a store-level regression of CLV on $K$, $W$, and $K \times W$); (ii)~a competitive multi-firm extension; (iii)~stochastic demand forcing, which would transform the Cliff into a probabilistic threshold. The model treats $F$ as exogenous; endogenising it would enable optimal deployment sequencing. The omission of a direct $W \to D$ channel makes our results a conservative lower bound on true complementarity.
