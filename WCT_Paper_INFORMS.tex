...
With only $G = 13$ (US) and $G = 11$ (EU) firm clusters, standard clustered standard errors are biased downward. Following \citet{cameron2008bootstrap}, we report $p$-values from the $t(G{-}1)$ distribution throughout and supplement with CR2 finite-sample corrections \citep{bell2002bias} and Webb six-point wild cluster bootstrap \citep{webb2023reworking} in the EC. All corrections are applied uniformly across both geographies.
...
\begin{table}[htbp]
\TABLE
{US regression results. Dependent variable: ACSI score (0--100). All $p$-values use $t(G{-}1)$ correction with $G = 13$--$17$ clusters. Significance: $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.10$.\label{tab:us_results}}
{\small
\begin{tabular}{lcccc}
\toprule
...
The European sample provides an independent out-of-sample test. Using Trustpilot (1--5 scale) as the outcome, the between-effects estimate of $\beta_3$ is positive ($+$0.47) and significant under pooled cluster-robust inference (CR2: $p = 0.045$ at $t(10)$). However, the conservative HC3 between-effects estimate ($p = 0.53$) and Webb bootstrap ($p = 0.62$) do not reject zero, reflecting the extreme power limitation of $G = 11$ clusters. The qualitative sign consistency across geographies---positive $K \times W$ in cross-section---provides suggestive, though not conclusive, support for complementarity.

\subsection{Limitations of the Empirical Analysis}\label{sec:emp:limits}

Four limitations warrant emphasis.
First, with $G \in \{11, 13\}$ clusters, clustered inference is at the boundary of asymptotic validity; we report wild bootstrap $p$-values in the EC for all key coefficients.
