...
Firm-level panel data on 13~US retailers (2010--2024) provide suggestive cross-sectional support: the technology--workforce interaction is positive in between-effects estimation, and an inverted-U between e-commerce share and customer satisfaction aligns with the predicted digitalisation ceiling. Causal identification awaits store-level data.
...
The model implies three cross-sectional patterns at the firm--year level: (i)~a positive technology--workforce interaction ($K \times W > 0$); (ii)~an inverted-U between e-commerce intensity and customer satisfaction; (iii)~a positive effect of workforce improvement on satisfaction that increases with technology intensity. We examine the first two using a panel of 13 US retailers. The third requires higher-frequency, store-level data that our annual firm-level proxies cannot deliver; we report it as a data-limitation disclosure rather than a test. An independent European sample ($N_c = 11$, $N = 20$) is reported in the EC (\S EC.5) for transparency; its extreme power limitation precludes formal inference.
...
\end{multline}
Prediction: $\beta_3 > 0$. Because the model's complementarity prediction concerns \emph{structural} differences across firms (a firm with jointly higher $K$ and $W$ should exhibit higher satisfaction), the between-effects (OLS) specification is the primary estimand. Within-firm fixed-effects specifications are reported as robustness checks, but we note ex ante that Glassdoor ratings exhibit low within-firm temporal variation (within-firm SD is only 30\% of total SD), making the FE estimator poorly suited to detecting a structural cross-firm relationship. Controls $\mathbf{X}_{it}$ include log revenue, and in extended specifications, SGA intensity and net margin.

\smallskip
\noindent\textit{Pattern 2 (Front-Stage Trap $\to$ Theorem~\ref{thm:trap}).}
...
\smallskip
\noindent\textit{Pattern 3 (Good Jobs Buffer $\to$ Theorem~\ref{thm:buffer})---data-limitation disclosure.}
The model predicts $\beta_2 > 0$ in
...
\end{multline}
However, the capacity-multiplier channel operates on monthly timescales (the model's $\tau_r \approx 1.7$ months), whereas annual Glassdoor ratings capture a lagged, smoothed signal. The first-differenced design, combined with few complete observations ($N = 84$--118), is structurally underpowered to detect this pattern with firm-level proxies. We report the specification for transparency but do not interpret it as a meaningful test; a store-level panel with high-frequency engagement data is the appropriate design (see~\S\ref{sec:disc:limits}).

\medskip
\noindent\textit{Inference corrections.}
With only $N_c = 13$ firm clusters, standard clustered standard errors are biased downward \citep{cameron2008bootstrap}. We adopt three progressively conservative corrections. First, all reported $p$-values use the $t(N_c{-}1)$ distribution rather than the standard normal. Second, we supplement with CR2 finite-sample bias-corrected standard errors \citep{bell2002bias}. Third, we report Webb six-point wild cluster bootstrap $p$-values \citep{webb2023reworking} in the EC. The most conservative inference (Webb bootstrap) is used as the primary benchmark for significance claims.
...
\midrule
\multicolumn{5}{l}{\textit{Pattern 1: Complementarity (between-effects primary)}} \\
$K^z$ & 0.2903 & $-$0.2153 & $-$0.4218 & $-$0.3102 \\
$W^z$ & 1.2506 & 0.1087 & 0.3181 & 0.4553 \\
$K \times W^z$ & 1.9533 & $-$0.6342$^{**}$ & $-$0.5615$^{*}$ & $-$0.7649 \\
$N$ & 131 & 131 & 131 & 83 \\
\midrule
\multicolumn{5}{l}{\textit{Pattern 2: Front-Stage Trap}} \\
...
$N$ & 216 & 216 & 190 & \\
\midrule
\multicolumn{5}{l}{\textit{Pattern 3: Good Jobs Buffer (data-limitation disclosure)}} \\
...
{$N$ varies across patterns because each requires different variable coverage: Pattern~1 requires ACSI, Glassdoor, and CapEx; Pattern~2 requires only ACSI and e-commerce share; Pattern~3 uses first-differenced variables, losing one observation per firm.}
\end{table}

\noindent\textit{Pattern 1 (Complementarity).} The between-effects (OLS) estimate of $\beta_3$ is positive ($+$1.95), consistent with the model's prediction that firms with jointly higher technology and workforce investment enjoy higher satisfaction. This finding is economically meaningful: a one-standard-deviation joint increase in $K$ and $W$ is associated with a 1.95-point ACSI gain, comparable in magnitude to the main effects individually. The direction aligns with the complementarity evidence in \citet{aral2012three}, who report similar positive cross-terms for technology--human-resource interactions in a different (non-retail) setting.

The within-firm estimates (Models 1b--1d), reported as robustness checks, are negative and sometimes significant. As anticipated (\S\ref{sec:emp:id}), firm fixed effects absorb the cross-sectional variation that identifies complementarity: within-firm Glassdoor SD is only 30\% of total SD, leaving insufficient temporal variation for within-firm identification. The sign reversal under fixed effects is a well-documented feature of complementarity tests when the identifying variation is predominantly cross-sectional \citep{milgrom1990economics, aral2012three}. The between-effects estimate is the appropriate estimand for a structural cross-firm relationship, though it is subject to omitted-variable bias; the small number of clusters ($N_c = 13$) limits the reliability of all specifications.

\noindent\textit{Pattern 2 (Front-Stage Trap).} The quadratic specification (2b) yields $\hat{\beta}_2 = -0.028$ ($p = 0.028$ under $t(16)$), consistent with the predicted inverted-U. The estimated turning point of 7.9\% e-commerce share corresponds to the early-to-mid 2010s transition, broadly consistent with the model's $F_{\mathrm{crit}} \approx 1.11$ threshold. The coefficient attenuates when workforce controls are included (Model~2c, $p = 0.31$); this attenuation is predicted by the model, which implies that e-commerce intensity damages satisfaction \emph{through} workforce depletion, so controlling for $W$ should absorb part of the quadratic effect. The turning point (7.9\%) is plausible given that US e-commerce share crossed this threshold around 2016 (FRED series ECOMPCTSA), a period when several major retailers reported significant click-and-collect growing pains \citep{hubner2021digitalization}.

\noindent\textit{Pattern 3 (Good Jobs Buffer---data limitation).} As anticipated (\S\ref{sec:emp:id}), the interaction $\Delta W \times K$ is negative in both specifications ($-41.40$, $-132.37$), opposite to the prediction. This result is uninformative rather than disconfirming: the first-differenced design with annual Glassdoor ratings operates at the wrong frequency for the capacity-multiplier channel, and the complete-case sample ($N = 84$--118) is severely underpowered. We do not interpret this specification as evidence for or against the model. Store-level identification with high-frequency engagement data is the natural next step (\S\ref{sec:disc:limits}).

\subsection{Assessment}\label{sec:emp:assessment}

The firm-level evidence provides suggestive but partial support for the model's cross-sectional patterns. The inverted-U (Pattern~2) is the strongest finding---statistically significant and economically plausible. The between-effects complementarity (Pattern~1) is directionally consistent and economically meaningful, but vulnerable to omitted-variable bias. The capacity-multiplier test (Pattern~3) is structurally underpowered at the firm--year level. The empirical contribution is therefore one of directional consistency rather than causal identification: the data do not contradict the model's qualitative predictions, but the primary contribution of the paper remains the analytical framework. Store-level identification (\S\ref{sec:disc:limits}) is the critical empirical follow-up.
...
\textit{Empirical limitations.} The firm-level panel analysis (\S\ref{sec:empirics}) provides suggestive cross-sectional evidence rather than causal identification. Four specific concerns warrant emphasis.
First, with only $N_c = 13$ firm clusters, clustered inference is at the boundary of asymptotic validity \citep{cameron2008bootstrap}; we report Webb six-point wild cluster bootstrap $p$-values in the EC for all key coefficients \citep{webb2023reworking}.
Second, Glassdoor ratings exhibit low within-firm temporal variation (within-firm SD is only 30\% of total SD), making the complementarity interaction difficult to identify within firm over time; the between-effects estimate is our primary specification but faces omitted-variable concerns.
Third, the complementarity interaction reverses sign under firm fixed effects (Models 1b--1d), consistent with limited within-firm variation rather than absence of the effect; store-level panel data with exogenous technology rollout events would permit sharper separation of the capacity-multiplier channel from confounds.
