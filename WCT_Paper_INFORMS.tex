...
Three empirical tests provide convergent support. An inverted-U between e-commerce share and customer satisfaction ($p = 0.028$) and a post-shock ACSI decline following major omnichannel launches ($p = 0.037$) support the digitalisation ceiling. County-level BLS data ($N = 30{,}876$ US counties, 2015--2024) confirm the Good Jobs Buffer: counties with higher initial retail wages show significantly more resilient employment as e-commerce grows ($p = 0.008$).
...
We examine the model's predictions using three complementary identification strategies at different levels of aggregation: (i)~firm-level cross-sectional and event-study evidence using ACSI and Glassdoor data ($N = 131$--$216$, 13--17 firms); (ii)~county-level panel evidence using BLS retail employment and wages ($N = 30{,}876$, 3{,}115 US counties). The firm-level analysis tests the Front-Stage Trap (inverted-U in e-commerce) and complementarity (technology--workforce interaction). The county-level analysis tests the Good Jobs Buffer at a different margin: whether counties with higher initial retail wages show more resilient retail employment as e-commerce penetration grows. An independent European sample ($N_c = 11$, $N = 20$) is reported in the EC (\S EC.5) for transparency.

\subsection{Data and Samples}\label{sec:emp:data}

\textit{Firm-level sample.}
A balanced panel of 13 publicly listed US retailers observed annually from 2010 to 2024. Customer satisfaction ($T$ proxy) is measured by the ACSI \citep{fornell1996acsi}. Workforce well-being ($W$ proxy) uses Glassdoor overall ratings (1--5 scale) \citep{corbett2024wellbeing}. Technology investment ($K$ proxy) is CapEx/Revenue from SEC EDGAR 10-K filings. E-commerce share is from FRED series ECOMPCTSA. After listwise deletion, the regression sample contains 131 complete firm--year observations.

\textit{County-level sample.}
BLS Quarterly Census of Employment and Wages (QCEW) provides county-level annual average retail employment, weekly wages, and establishment counts for NAICS 44--45 (Retail Trade), 2015--2024. We retain counties with baseline retail employment exceeding 50 and weekly wage exceeding \$200, yielding 30{,}876 county--year observations across 3{,}115 counties.
...
\noindent\textit{Event study (Theorem~\ref{thm:trap}).} We exploit the staggered timing of major omnichannel launches (e.g., Walmart curbside 2017, Target Drive Up 2018, Kroger ClickList 2016) as firm-level technology shocks.  Using a $[-3, +3]$ year event window around each firm's launch ($N = 108$, 16 firms), a two-way FE specification with pre-shock CapEx as control yields a post-shock ACSI decline of $-0.71$ ($p = 0.037$ under $t(15)$), consistent with the Front-Stage Trap: major omnichannel deployments reduce customer satisfaction on average. Pre-trend coefficients are near zero and insignificant, supporting the parallel-trends assumption. The workforce moderation (post $\times$ pre-Glassdoor) is not detectable ($p = 0.79$), consistent with the low within-firm Glassdoor variation rather than absence of the mechanism.

\subsection{County-Level Good Jobs Buffer}\label{sec:emp:county}

The firm-level data cannot deliver a well-powered test of the capacity-multiplier channel. We therefore test the Good Jobs Buffer at a different margin using BLS county-level data, where the dramatically larger sample ($N = 30{,}876$) provides the statistical power that the firm-level analysis lacks.

\textit{Design.} The model predicts that workforce investment buffers the negative effects of digitalisation on service capacity. We test a county-level analogue: do counties with higher initial retail wages (a workforce-investment proxy) show more resilient retail employment as national e-commerce penetration grows? Initial retail wage is measured in 2015 (before the e-commerce acceleration), standardised cross-sectionally. The interaction $W_{c,2015}^z \times \text{Ecom}_t$ captures whether high-wage counties differentially maintain employment under digital pressure:
\begin{equation}\label{eq:county}
\ln E_{ct} - \ln E_{c,2015} = \alpha_c + \gamma_t + \beta_1 (W_{c,2015}^z \times \text{Ecom}_t) + \varepsilon_{ct}.
\end{equation}

\textit{Results.} The interaction is positive and significant ($\hat{\beta}_1 = 0.0027$, $t = 2.65$, $p = 0.008$; county and year FE, clustered at county). A county one SD above the mean in initial retail wage (${\approx}\$110$/week) retains 2.7 percentage points more retail employment per 10-point increase in e-commerce share, consistent with the Good Jobs Buffer operating through employment capacity. The long-difference specification (2015 vs.\ 2023) confirms: high-wage counties gained 3.5~pp more retail employment over the period ($t = 3.81$, $p < 0.001$).

\textit{Robustness.} The result strengthens when COVID years are excluded ($\hat{\beta}_1 = 0.0032$, $p = 0.003$), holds in urban counties ($p = 0.009$), and is marginally significant in rural counties ($p = 0.068$). A pre-period placebo (2015--2017, when e-commerce grew modestly) shows no effect ($p = 0.44$), confirming that the wage buffer activates only under substantial digital pressure. The effect operates through employment quantity, not wage levels: using wage growth as the dependent variable yields a null result ($p = 0.79$), consistent with the model's capacity-multiplier channel.

\subsection{Assessment}\label{sec:emp:assessment}

Three findings across two levels of analysis support the model's qualitative predictions. First, the inverted-U between e-commerce and ACSI (Pattern~2, $p = 0.028$) and the post-shock ACSI decline in the event study ($p = 0.037$) both support the Front-Stage Trap. Second, the county-level wage--e-commerce interaction ($p = 0.008$, $N = 30{,}876$) provides a well-powered test of the Good Jobs Buffer at the employment-capacity margin. Third, the between-effects complementarity ($+1.95$) is directionally consistent but imprecisely estimated. The empirical contribution is one of convergent evidence across different identification strategies, levels of analysis, and outcome variables, subject to the limitations in \S\ref{sec:disc:limits}.
