...
\textbf{Loop R1} ($T \to D \to G \to T$, reinforcing): trust raises demand, which changes the service gap, feeding back to trust.
...
\begin{tikzpicture}[
    ...
    \node[looplabel, text=orange!80!black, draw=orange!80!black] at (5.5, 2.2) {R1 $(+)$};
...
\medskip
Under a fixed budget constraint $c_F F + c_f f \leq B$, the CLV-maximising allocation always includes positive workforce investment $f^* > 0$: the all-$F$ corner is strictly dominated whenever the budget reaches or exceeds the Trap threshold (proof in EC~\S EC.1.7).
...
{The hidden cost of app innovation. Panel~(a): front-stage KPIs rise for all three scenarios. Panel~(b): CLV diverges sharply by month~24. Panels~(c,d): workforce collapse precedes trust collapse in the no-investment scenario.\label{fig:hidden_cost}}
{}
\end{figure}
...
The European sample provides an independent out-of-sample test. Using Trustpilot (1--5 scale) as the outcome, the between-effects estimate of $\beta_3$ is positive ($+$0.47) and significant under pooled cluster-robust inference (CR2: $p = 0.045$ at $t(10)$). However, the conservative HC3 between-effects estimate ($p = 0.53$) and Webb bootstrap ($p = 0.62$) do not reject zero, reflecting the extreme power limitation of $N_c = 11$ clusters \citep{cameron2008bootstrap}. The qualitative sign consistency across geographies---positive $K \times W$ in cross-section---provides suggestive, though not conclusive, support for complementarity. That the direction replicates in an independent sample with different outcome measures (Trustpilot vs.\ ACSI), different labour market institutions, and different retail structures is encouraging, even if the statistical precision is insufficient for formal inference.
...
Third, the EU outcome (Trustpilot) is complaint-biased and operates on a different scale than ACSI \citep{mittal1998asymmetric}; while we rescale to 0--100 for coefficient comparability, measurement equivalence is not guaranteed.
