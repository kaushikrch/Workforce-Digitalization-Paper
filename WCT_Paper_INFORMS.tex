...
We formalise this gap as a parsimonious dynamic model and derive three analytically characterised results. (i)~A computable digitalisation ceiling exists beyond which additional front-stage investment destroys customer lifetime value (CLV) by overloading back-stage workers; revenue metrics continue rising even as CLV collapses. (ii)~The entire return to workforce investment flows through service capacity, invisible to dashboards that treat payroll as a cost line. (iii)~The system is bistable: the neglected firm faces a safe rollout window less than two-fifths the size of its investing peer. We provide suggestive empirical support using panel data on 13 US and 11 European retailers (2010--2024): the technology--workforce interaction is positive in cross-section, and an inverted-U between e-commerce share and customer satisfaction aligns with the predicted digitalisation ceiling.
...
All variables are standardised to mean zero and unit standard deviation before regression. We estimate three specifications mapping to the three model predictions:

\smallskip
\noindent\textit{Test 1 (Complementarity $\to$ Proposition~\ref{prop:complementarity}).}
\begin{equation}\label{eq:reg1}
T_{it} = \alpha_i + \gamma_t + \beta_1 K_{it}^z + \beta_2 W_{it}^z + \beta_3 (K \times W)_{it}^z + \mathbf{X}_{it}\boldsymbol{\delta} + \varepsilon_{it}.
\end{equation}
...
\Delta T_{it} = \alpha_i + \gamma_t + \beta_1 \Delta W_{it} + \beta_2 (\Delta W \times K)_{it} + \mathbf{X}_{it}\boldsymbol{\delta} + \varepsilon_{it}.
\end{equation}
Prediction: $\beta_2 > 0$ (workforce improvement matters more at high technology intensity).

\medskip
\noindent\textit{Inference corrections.}
With only $N_c = 13$ (US) and $N_c = 11$ (EU) firm clusters, standard clustered standard errors are biased downward. Following \citet{cameron2008bootstrap}, we report $p$-values from the $t(N_c{-}1)$ distribution throughout and supplement with cluster-robust (CR2) finite-sample corrections \citep{bell2002bias} and Webb six-point wild cluster bootstrap \citep{webb2023reworking} in the EC. All corrections are applied uniformly across both geographies.
...
\noindent\textit{Test 1 (Complementarity).} The cross-sectional (OLS) estimate of $\beta_3$ is positive ($+$1.95), consistent with the model's prediction that firms with jointly higher technology and workforce investment enjoy higher satisfaction. However, the within-firm estimates (Models 1b--1d) are negative and sometimes significant, suggesting that year-to-year co-movements in $K$ and $W$ within the same firm do not raise satisfaction. This reversal is consistent with low within-firm variation in Glassdoor ratings (within-firm SD is only 30\% of total SD): firm fixed effects absorb the cross-sectional signal that drives complementarity.

\noindent\textit{Test 2 (Front-Stage Trap).} The quadratic specification (2b) yields $\hat{\beta}_2 = -0.028$ ($p = 0.028$ under $t(16)$), confirming the predicted inverted-U. The estimated turning point of 7.9\% e-commerce share corresponds to the early-to-mid 2010s transition, broadly consistent with the model's $F_{\mathrm{crit}} \approx 1.11$ threshold.

\noindent\textit{Test 3 (Good Jobs Buffer).} The interaction $\Delta W \times K$ is negative in both specifications, contrary to the prediction. The first-differenced design, combined with noisy annual Glassdoor ratings and few complete observations ($N = 84$--118), leaves this test underpowered.

\medskip
\noindent\textit{EU replication.}
The European sample provides an independent out-of-sample test. Using Trustpilot (1--5 scale) as the outcome, the between-effects estimate of $\beta_3$ is positive ($+$0.47) and significant under pooled cluster-robust inference (CR2: $p = 0.045$ at $t(10)$). However, the conservative HC3 between-effects estimate ($p = 0.53$) and Webb bootstrap ($p = 0.62$) do not reject zero, reflecting the extreme power limitation of $N_c = 11$ clusters. The qualitative sign consistency across geographies---positive $K \times W$ in cross-section---provides suggestive, though not conclusive, support for complementarity.

\subsection{Limitations of the Empirical Analysis}\label{sec:emp:limits}

Four limitations warrant emphasis.
First, with $N_c \in \{11, 13\}$ clusters, clustered inference is at the boundary of asymptotic validity; we report wild bootstrap $p$-values in the EC for all key coefficients.
Second, Glassdoor ratings exhibit low within-firm temporal variation, making the complementarity interaction difficult to identify within firm over time; between-effects estimates (which exploit cross-sectional variation) are more informative but face omitted-variable concerns.
Third, the EU outcome (Trustpilot) is complaint-biased and operates on a different scale than ACSI; while we rescale to 0--100 for coefficient comparability, measurement equivalence is not guaranteed.
Fourth, the data are at the firm--year level; the model's predictions concern service-capacity dynamics that are most cleanly testable with store-level panel data and exogenous technology rollout events.
These results are best interpreted as suggestive evidence consistent with the model's qualitative predictions, pending the store-level identification strategy outlined in \S\ref{sec:disc:limits}.
...
\textit{Model limitations.} The model treats $F$ as exogenous; endogenising it would enable optimal deployment sequencing. The omission of a direct $W \to D$ channel makes our results a conservative lower bound on true complementarity.

\textit{Empirical limitations.} The firm-level panel analysis (\S\ref{sec:empirics}) provides suggestive but not definitive support. With only 13 (US) and 11 (EU) firm clusters, all clustered inference operates at the boundary of asymptotic validity. The complementarity interaction reverses sign under firm fixed effects, consistent with limited within-firm temporal variation in Glassdoor ratings rather than absence of the effect. Store-level panel data with exogenous technology rollout events---the gold-standard identification strategy---would permit sharper separation of the capacity-multiplier channel from confounds.
