...
\end{table}; qualitative behaviour is identical. With the three restrictions lifted, the model generates Loops R1, R2, and the cross-domain Front-Stage Trap.
...
Seven checks supplement the core results: (i)~between-effects OLS, (ii)~Mundlak CRE, (iii)~one-year lag ($K_{t-1}^z \times W_{t-1}^z$), (iv)~COVID exclusion, (v)~leave-one-out, (vi)~wild cluster bootstrap (9{,}999 reps, Webb six-point weights; \citealp{webb2023reworking}), and (vii)~CR3 small-sample correction ($\mathrm{df} = G{-}k = 9$). All checks preserve the positive between-effects sign.
