...
\textit{Interior sub-case (computational).} For $B \in [B_{\min}/2,\, B_{\min}-\varepsilon)$, strict dominance of $f$ holds computationally across cost ratios $c_F/c_f \in [0.5, 5]$ (at baseline values of all other parameters; the range spans realistic investment cost structures).
...
All time units are months; rate parameters are in months$^{-1}$ with half-lives $\ln(2)/\theta$. The calibration follows \citet{homer1993partial} and \citet{barlas1996formal}. Three parameters warrant additional discussion beyond Table~\ref{tab:ec:paramsource}.
...
$\sigma$ (overload asym.) & 2.00 & -- & \citeauthor{oliva2001cutting} (\citeyear{oliva2001cutting}); \citet{bakker2007job} & Cliff disappears at $\sigma{=}1$ \\
...
\bottomrule
\end{tabular}}}
{$^{\dagger}$Balance manifold: $K_0 h(W_0)=(1{+}a{\cdot}0.5)F^b$, $T_0{=}0.50$. Grid: $37{\times}26$ (962~pts). Collapse: $T(500){<}0.15$. $^{\ddagger}$Inherited from the starting equilibrium ($u = 0.05$).}
\end{table}
...
Seven robustness checks supplement the core results. (i)~Between-effects OLS ($K^z \times W^z = +1.95$, $p = 0.41$), reported as the structural estimand in the main text. (ii)~Mundlak correlated random effects, which adds within-firm means as controls. (iii)~One-year lag specification using $K_{t-1}^z \times W_{t-1}^z$ to address reverse causality. (iv)~COVID exclusion (dropping 2020--2021). (v)~Leave-one-out sensitivity, cycling each of the 13 firms out of the sample. (vi)~Wild cluster bootstrap with 9{,}999 replications using the six-point Webb weights \citep{webb2023reworking}. (vii)~CR3 small-sample correction with $\mathrm{df} = G - k = 9$. Results are qualitatively stable across all seven checks; the between-effects estimate remains positive throughout.
...
An independent panel covers 11 European retailers (UK, France, Germany, Spain, Sweden, Netherlands; 2019--2023; $N{=}20$ complete firm--year observations). Customer satisfaction uses Trustpilot (1--5); workforce well-being uses Glassdoor; financials from Yahoo Finance with asset percentile rank as currency-neutral control. E-commerce share from UK ONS (J4MC) and Eurostat (\texttt{isoc\_ec\_eseln2}).

\textit{EU results.}
The between-effects $\beta_3$ ($K{\times}W$) is positive ($+$0.47; CR2: $p = 0.045$) but fails robustness checks (HC3: $p = 0.53$; Webb bootstrap \citep{webb2023reworking}: $p = 0.62$), reflecting $N_c = 11$ clusters \citep{cameron2008bootstrap}. These results are reported as exploratory. Additional robustness checks include currency-neutral controls (asset percentile rank), UK-only and Continental sub-samples, leave-one-out sensitivity (interaction estimates ranging from $-0.04$ to $+0.82$), and Imbens--Kolesar small-sample correction ($\mathrm{df}_{\mathrm{eff}}{=}7.2$, $p{=}0.055$).
}% end \small
