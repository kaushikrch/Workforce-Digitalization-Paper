...
\textit{EU results.}
The between-effects $\beta_3$ ($K{\times}W$) is positive ($+$0.47; CR2: $p = 0.045$) but fails robustness checks (HC3: $p = 0.53$; Webb bootstrap \citep{webb2023reworking}: $p = 0.62$), reflecting $N_c = 11$ clusters \citep{cameron2008bootstrap}. Reported as exploratory. Additional checks: currency-neutral controls; UK-only/Continental splits; leave-one-out ($K{\times}W \in [-0.04, +0.82]$); Imbens--Kolesar $\mathrm{df}_{\mathrm{eff}}{=}7.2$, $p{=}0.055$.
}% end \small
