...
\]
Trust declines monotonically. Through Loop R2 ($W\!\downarrow \to h(W)\!\downarrow \to G\!\uparrow$), the gap widens, accelerating trust decline. The convexity of demand in $F$ ($\partial^2 D/\partial F^2 > 0$ for $b > 1$) ensures the gap widens super-linearly, making the collapse self-reinforcing under Loop R2; convergence to the low-trust attractor is confirmed numerically across the studied parameter configurations (Tier~C; main paper Remark~1).% NB: hardcoded cross-document ref; verify against main paper Remark numbering The system converges to the low-trust interior attractor (Remark~\ref{rem:low_trust}).
...
\textbf{Overload-recovery asymmetry ($\sigma = 2.0$).} Magnitude comparator from \citet{oliva2001cutting}; independently consistent with the loss-aversion ratio $\lambda \approx 2.25$ from prospect theory \citep{tversky1992advances}. The Cliff disappears only at $\sigma = 1$ (Table~\ref{tab:ec:sensitivity}).
