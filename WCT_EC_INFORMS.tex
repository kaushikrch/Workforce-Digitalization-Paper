...
Customer satisfaction uses ACSI scores (0--100; \texttt{theacsi.org}); workforce well-being uses Glassdoor overall ratings (1--5; minimum five reviews per firm--year); financial data are from SEC EDGAR 10-K filings (XBRL API); e-commerce share from FRED (ECOMPCTSA); labour-market controls from BLS JOLTS. Model proxies: $K =$ CapEx/Revenue, $W =$ Glassdoor overall, $T =$ ACSI, $K \times W =$ standardised interaction.

\textit{US merge.}
The panel is constructed from an EDGAR backbone of 276 observations across 17 firms (2008--2024), left-joined with ACSI, Glassdoor, and macro controls, yielding 131 complete firm--year observations (47\% coverage). All merges enforce one-to-one key uniqueness.

\textit{US robustness.}
Seven checks supplement the core results: (i)~between-effects ($K \times W = +1.95$, $p = 0.41$); (ii)~Mundlak CRE; (iii)~lagged $K_{t-1} \times W_{t-1}$; (iv)~COVID exclusion; (v)~leave-one-out; (vi)~wild bootstrap (9{,}999 reps); (vii)~CR3 with $\mathrm{df} = G - k = 9$.
