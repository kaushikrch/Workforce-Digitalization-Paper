...
Six robustness checks supplement the core between-effects OLS estimand (reported in \S 5.2 of the main paper): (i)~Mundlak CRE, (ii)~one-year lag ($K_{t-1}^z \times W_{t-1}^z$), (iii)~COVID exclusion (dropping 2020--2021), (iv)~leave-one-out, (v)~wild cluster bootstrap (9{,}999 reps, Webb six-point weights; \citealp{webb2023reworking}), and (vi)~CR3 small-sample correction ($\mathrm{df} = G{-}k = 9$). All checks preserve the positive sign.
