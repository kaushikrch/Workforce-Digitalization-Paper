...
\]
Trust declines monotonically. Through Loop R2 ($W\!\downarrow \to h(W)\!\downarrow \to G\!\uparrow$), the gap widens, accelerating trust decline. The convexity of demand in $F$ ($\partial^2 D/\partial F^2 > 0$ for $b > 1$) ensures the gap widens super-linearly, making the collapse self-reinforcing under Loop R2; convergence to the low-trust attractor is confirmed numerically across the studied parameter configurations (Tier~C; main paper Remark~\ref{rem:transparency}). The system converges to the low-trust interior attractor (Remark~\ref{rem:low_trust}).
...
Table~\ref{tab:ec:pretrends} reports event-time coefficients from the level DID and workforce-moderation specifications described in \S5.2 of the main paper ($N = 108$, 16 firms, $[-3,+3]$ window around each firm's omnichannel launch). Pre-period coefficients ($t=-3$, $t=-2$) are uniformly small and insignificant in both specifications, supporting the parallel-trends assumption.
...
\textit{EU data sources.}
Trustpilot (1--5, complaint-biased); Glassdoor (1--5); Yahoo Finance (\texttt{yfinance}); UK ONS internet sales (J4MC); Eurostat (\texttt{isoc\_ec\_eseln2}). Assets rank${=}$within-year percentile (currency-neutral).

\textit{EU merge.}
yfinance backbone (50 obs, 11 firms; 4 duplicate rows dropped). Trustpilot/Glassdoor merged with 1:1 validation. Complete: 20 obs (40\%). Fiscal-year offsets ${\leq}$3 months (Tesco Feb, Next Jan, ABF Sep, H\&M Nov).

\textit{EU results.}
Using Trustpilot (1--5 scale) as the outcome, the between-effects estimate of $\beta_3$ is positive ($+$0.47) and significant under pooled cluster-robust inference (CR2: $p = 0.045$ at $t(10)$). However, the conservative HC3 between-effects estimate ($p = 0.53$) and Webb bootstrap ($p = 0.62$) do not reject zero, reflecting the extreme power limitation of $N_c = 11$ clusters \citep{cameron2008bootstrap}. The qualitative sign consistency across geographies---positive $K \times W$ in cross-section---provides suggestive, though not conclusive, support for complementarity. With only $N{=}20$ observations and $N_c{=}11$ clusters, this sample is below any standard threshold for panel regression and is reported as an exploratory out-of-sample check rather than for inferential purposes.
...
(v)~CR2, $t(10)$: $p{=}0.045$;
(vi)~Imbens--Kolesar $\mathrm{df}_{\mathrm{eff}}{=}7.2$, $p{=}0.055$.
}% end \small
