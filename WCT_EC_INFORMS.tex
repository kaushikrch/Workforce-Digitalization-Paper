...
\begin{proof}
We verify each boundary by checking the sign of the directional derivative.

\textit{Workforce well-being.} At $W = 0$: $\dot{W} = (\Gm + f)(1 - 0) - (\mu + \sigma\Gp)\cdot 0 = \Gm + f \geq 0$, since $\Gm \geq 0$ and $f \geq 0$. Hence $W$ cannot exit below zero. At $W = 1$: $\dot{W} = (\Gm + f)(1 - 1) - (\mu + \sigma\Gp)\cdot 1 = -(\mu + \sigma\Gp) \leq 0$, since $\mu > 0$ and $\sigma, \Gp \geq 0$. Hence $W$ cannot exit above one.

\textit{Customer trust.} At $T = 0$: $\dot{T} = \beta\Gm(1 - 0) - \eta\beta\Gp\cdot 0 - \lambda\cdot 0 = \beta\Gm \geq 0$. At $T = 1$: $\dot{T} = \beta\Gm\cdot 0 - \eta\beta\Gp\cdot 1 - \lambda\cdot 1 = -(\eta\beta\Gp + \lambda) \leq 0$.

\textit{Capability stock.} At $K = 0$: $\dot{K} = r\Gp W - \delta\cdot 0 + u = r\Gp W + u \geq u > 0$. Thus $K$ cannot reach zero while $u > 0$.

In each case, the vector field points strictly into $\Omega$ (or is tangent to the boundary), so $\Omega$ is forward-invariant by the standard barrier argument. All state variables therefore remain in their economically meaningful domains for all $t \geq 0$.\Halmos
\end{proof}
...
For any $F < F_{\mathrm{crit}}$, a surplus equilibrium $(K^*, W^*, T^*)$ with $S^* > 0$ exists. Under Regularity Condition C2 (single-crossing condition C2-SC: $\beta(f+\mu)<\lambda$; analytic, see Step~3 above), this equilibrium is unique.
\end{lemma}

\begin{proof}
At the surplus equilibrium, $\Gp = 0$ and $\Gm = S^*$, so from (\ref{ec:dK}): $K^* = u/\delta$, which depends only on exogenous parameters. The remaining equilibrium conditions yield:
\[
W^*(S) = \frac{S + f}{S + f + \mu}, \qquad T^*(S) = \frac{\beta S}{\beta S + \lambda}.
\]
Both are continuous, strictly increasing functions of $S$ on $(0, \infty)$, with $W^*(0) = f/(f+\mu) \in (0,1)$, $W^*(S) \to 1$ as $S \to \infty$, $T^*(0) = 0$, and $T^*(S) \to 1$ as $S \to \infty$.

Define the surplus residual:
\begin{align}\label{ec:Phi}
\Phi(S) &= \frac{u}{\delta}\bigl[(1-p) + p\,W^*(S)\bigr] \notag\\
  &\quad -\; \bigl(1 + a\,T^*(S)\bigr)F^b \;-\; S.
\end{align}
The equilibrium condition $S^* = K^*h(W^*) - D(T^*, F)$ is equivalent to $\Phi(S^*) = 0$.

\textbf{Step 1 (Boundary behaviour).}
\[
\Phi(0) \;=\; \frac{u}{\delta}\Bigl[(1-p) + \frac{pf}{f+\mu}\Bigr] - F^b.
\]
This is strictly positive when $F < F_{\mathrm{crit}} = \bigl((u/\delta)\,h\bigl(f/(f+\mu)\bigr)\bigr)^{1/b}$, which is exactly the condition defining the surplus regime.

As $S \to \infty$: $W^*(S) \to 1$ and $T^*(S) \to 1$, so $\Phi(S) \to (u/\delta) - (1+a)F^b - S \to -\infty$, since the linear term $-S$ dominates.

\textbf{Step 2 (Continuity and existence).} Since $W^*(S)$ and $T^*(S)$ are continuous, $\Phi$ is continuous on $[0, \infty)$. By the Intermediate Value Theorem, $\Phi(0) > 0$ and $\Phi(S) \to -\infty$ imply the existence of at least one $S^* > 0$ with $\Phi(S^*) = 0$.

\textbf{Step 3 (Uniqueness).} Differentiating (\ref{ec:Phi}): $\Phi'(S) = \tau_1(S) - \tau_2(S) - 1$, where $\tau_1(S) = (u/\delta)\,p\mu\,(S{+}f{+}\mu)^{-2}$ and $\tau_2(S) = aF^b\beta\lambda\,(\beta S{+}\lambda)^{-2}$ are both strictly positive and decreasing.

\noindent\textit{Analytic sufficient condition (C2-SC): $\beta(f+\mu) < \lambda$.} Under C2-SC, the ratio $\tau_1(S)/\tau_2(S) \propto [(\beta S{+}\lambda)/(S{+}f{+}\mu)]^2$ is strictly decreasing in $S$: its derivative's numerator is proportional to $\beta(f{+}\mu)-\lambda < 0$. Since $\tau_2$ is strictly decreasing, $1/\tau_2$ is strictly increasing, so $1+1/\tau_2$ is strictly increasing. The equation $\Phi'(S)=0$ is equivalent to $\tau_1/\tau_2 = 1+1/\tau_2$: a strictly decreasing function equals a strictly increasing function, which has at most one solution. Combined with $\Phi(0)>0$ and $\Phi(S)\to-\infty$ (IVT gives existence), $\Phi$ therefore has exactly one positive root.

C2-SC holds throughout the empirically relevant parameter space: $\beta(f{+}\mu) \leq 0.12{\times}0.15 = 0.018 < 0.025 = \lambda$ for all $f\in[0,0.10]$, independent of $b$, $p$, and $\sigma$. \Halmos
\end{proof}

\begin{remark}[Low-Trust Interior Attractor]\label{rem:low_trust}
The model admits a second, low-trust fixed point whenever $F$ is sufficiently large. At this attractor, $T^* = 0$ (since $dT/dt = -T(\eta\beta G + \lambda) < 0$ for all $T > 0$ when $G > 0$). With $T^* = 0$, demand is $D = F^b$, and the workforce equation (\ref{ec:dW}) at steady state with $\Gp = G^* > 0$, $\Gm = 0$ gives
\[
    W^*_{\mathrm{low}} \;=\; \frac{f}{f + \mu + \sigma\, G^*},
\]
which is strictly positive for $f > 0$. Substituting into the capability equation yields $K^*_{\mathrm{low}} = (r\,G^*\,W^*_{\mathrm{low}} + u)/\delta$, with $G^*$ determined self-consistently from $G^* = F^b - K^*_{\mathrm{low}}\,h(W^*_{\mathrm{low}})$.

\textbf{Local stability.} The $3 \times 3$ Jacobian evaluated at the low-trust fixed point has all eigenvalues with strictly negative real parts, confirmed numerically across $F \in [1.0, 1.5]$. A sufficient condition for the stability of $W^*_{\mathrm{low}}$ is that the net depletion rate exceeds the sensing-driven recovery rate:
\[
    \mu + \sigma G^* \;>\; r\,W^*_{\mathrm{low}},
\]
which is satisfied at baseline (left side $\approx 0.29$, right side $\approx 0.06$ at $F = 1.2$) and holds robustly across the parameter space. The existence of this interior attractor jointly with the surplus equilibrium of Lemma~\ref{lem:existence} establishes the bistability invoked by Theorem~3.
\end{remark}
...
Lemma~\ref{lem:bounded} ensures all trajectories remain in a bounded compact set, a necessary condition for LaSalle's invariance principle. Which attractor a trajectory reaches depends on its initial condition relative to the basin boundary (Figure~2, main paper). For $F \geq F_{\mathrm{crit}}$, only the low-trust attractor is stable (Remark~\ref{rem:low_trust}). A complete global Lyapunov proof is left to future work: the principal obstacle is the switching manifold $\mathcal{M} = \{G=0\}$, at which the vector field is discontinuous and the standard LaSalle invariance argument does not apply directly. A complete proof would require either a Filippov-specific Lyapunov construction~\citep{filippov1988differential} or a smooth-approximation argument; both are non-trivial for three-dimensional piecewise-smooth systems and constitute a natural extension of the present framework.
\end{remark}

\begin{remark}[Maintained Conditions]\label{rem:conditions}
Four conditions are maintained throughout: (C1)~$\Phi(0) > 0$ (i.e., $F < F_{\mathrm{crit}}$; analytic via IVT); (C2)~uniqueness of $S^*$, established analytically under C2-SC ($\beta(f+\mu)<\lambda$, which holds for all $f\in[0,0.10]$ independent of $b$, $p$, $\sigma$; see Step~3 above); (C3)~regularity $\Phi'(S^*) < 0$, a consequence of~(C1)--(C2), required for the implicit function theorem in Theorems~1--2; and (C4)~low-trust attractor existence $F^b > (u/\delta)\,h(W^*_{\mathrm{low}})$, verified numerically (Jacobian eigenvalues across $F \in [1.0, 1.5]$).
\end{remark}

\begin{remark}[Bifurcation Classification at $F_{\mathrm{crit}}$]\label{rem:bifurcation}
The transition at $F_{\mathrm{crit}}$ is a saddle-node (fold) bifurcation in the effective one-dimensional dynamics of the surplus variable $S$: the stable surplus equilibrium and the separating saddle coalesce and disappear as $F \nearrow F_{\mathrm{crit}}$. The Jacobian of the full system at the surplus equilibrium has one eigenvalue $-\delta < 0$ (decoupled $K$-equation) and two eigenvalues from the $(W,T)$ sub-system with strictly negative real parts throughout $F \in [0, F_{\mathrm{crit}}]$, confirmed numerically (baseline: two complex-conjugate eigenvalues with real part $\approx -0.07$; constant term of the characteristic polynomial $\approx 0.021 > 0$ at $F = 0.90$). No Hopf bifurcation occurs: both eigenvalues retain strictly negative real parts up to and including the limit $S^* \to 0$ (constant term $\to 0.004 > 0$ at $F_{\mathrm{crit}}$). The zero eigenvalue of the full system at $F_{\mathrm{crit}}$ belongs to the one-dimensional manifold along which $S^*$ vanishes, consistent with a saddle-node in the reduced dynamics.
\end{remark}
...
\]
Trust declines monotonically. Through Loop R2 ($W\!\downarrow \to h(W)\!\downarrow \to G\!\uparrow$), the gap widens, accelerating trust decline. The convexity of demand in $F$ ($\partial^2 D/\partial F^2 > 0$ for $b > 1$) ensures the gap widens super-linearly, making the collapse irreversible under the reinforcing loop R2. The system converges to the low-trust interior attractor (Remark~\ref{rem:low_trust}).
...
\begin{proof}
\textbf{Step 1: Two thresholds.} The permanent-collapse threshold follows directly from Theorem~1: for $F_0 + \Delta F \geq F_{\mathrm{crit}}$, equation~(\ref{ec:Phi}) at $S=0$ gives $\Phi(0) \leq 0$, so no surplus equilibrium exists. Therefore
\[
...
\textbf{Steps 3--4: Well-being dynamics and timescales.} Under overload ($G > 0$), the tipping threshold $W_{\mathrm{tip}} = f/(f+\mu+\sigma G_0^+)$ marks the level below which depletion exceeds recovery. In the slow-collapse zone ($G_0^+ = 0$ at $t_0^+$), $W_{\mathrm{tip}}$ is crossed gradually as surplus erodes. Two timescales govern speed: $\tau_{\mathrm{adapt}} = 1/r \approx 2.5$ mo (Loop B1) and $\tau_{\mathrm{deplete}} = 1/(\mu+\sigma G_0^+) \in [1.5,4]$ mo (Loop R3). Near $\Delta F_{\mathrm{collapse}}$, both are comparable, so convergence to the low-trust attractor may require hundreds of months (explaining why simulated cliffs for low-$f$ scenarios exceed the analytic threshold — a slow-convergence artefact, not a model error).

\textbf{Step 5: Three-threshold characterisation and numerical verification.} Starting from the surplus equilibrium $(K^*, W^*, T^*)$ at $F_0$, a step upgrade $\Delta F$ produces the following structure. Let $\Delta F_{\mathrm{collapse}} = F_{\mathrm{crit}}(f) - F_0$ be the saddle-node threshold (Theorem~1) and let $\Delta F_{\mathrm{rapid}} = S_0 / [b F_0^{b-1}(1+aT^*)]$ be the instantaneous-overload threshold. At the baseline scenario ($f=0.03$: $0.401 > 0.310$), $\Delta F_{\mathrm{rapid}} > \Delta F_{\mathrm{collapse}}$ and they define three distinct zones:
\begin{itemize}
    \item \textbf{Safe zone} ($\Delta F < \Delta F_{\mathrm{collapse}}$): surplus equilibrium exists at $F_0+\Delta F$; trajectories recover after transient overload.
    \item \textbf{Slow-collapse zone} ($\Delta F_{\mathrm{collapse}} \leq \Delta F < \Delta F_{\mathrm{rapid}}$): surplus equilibrium gone; $G(t_0^+) < 0$, so collapse proceeds gradually; convergence to the low-trust attractor may require hundreds of months near the boundary.
    \item \textbf{Rapid-collapse zone} ($\Delta F \geq \Delta F_{\mathrm{rapid}}$): $G(t_0^+) > 0$ immediately; the death spiral begins from $t=0^+$.
\end{itemize}
\noindent\textit{Note on ordering.} The ordering $\Delta F_{\mathrm{rapid}} > \Delta F_{\mathrm{collapse}}$ depends on $f$; it reverses at high $f$ (e.g., $f=0.10$: $\Delta F_{\mathrm{collapse}} = 0.49 > \Delta F_{\mathrm{rapid}} \approx 0.40$), where the slow-collapse zone disappears.
Simulations ($t_{\mathrm{end}}=500$ months, collapse criterion $T(\infty) < 0.15$; the gap between the two attractors renders this threshold unambiguous) confirm collapse boundaries for three workforce-investment scenarios starting from their surplus equilibria at $F_0=0.80$:


For $f=0.01$ the simulated cliff (0.27) exceeds $\Delta F_{\mathrm{collapse}}$ (0.179): the surplus equilibrium disappears at $\Delta F=0.179$, but the trajectory takes $>500$ months to converge for $\Delta F \in [0.179, 0.26]$, reflecting the long transients of the slow-collapse zone. For $f=0.03$, the simulated cliff (0.29) falls slightly \emph{below} $\Delta F_{\mathrm{collapse}}$ (0.310), consistent with basin-boundary effects: the trajectory escapes the surplus attractor's basin for $\Delta F \in [0.29, 0.31)$ even though the equilibrium formally exists.

\noindent\textit{Note on the two-threshold interpretation.} The Burnout Cliff involves two distinct thresholds that are easily conflated. $\Delta F_{\mathrm{collapse}} = F_{\mathrm{crit}}(f) - F_0$ is the \emph{safety boundary}: the maximum upgrade size beyond which no surplus equilibrium exists and permanent collapse is certain. $\Delta F_{\mathrm{rapid}}$ is a \emph{diagnostic}: the upgrade size above which the service gap turns positive at $t = 0^+$, triggering an immediate death spiral rather than a slow erosion. Because $\Delta F_{\mathrm{rapid}} > \Delta F_{\mathrm{collapse}}$ at the baseline scenario (0.40 vs.\ 0.31), a firm in the zone $\Delta F \in [\Delta F_{\mathrm{collapse}}, \Delta F_{\mathrm{rapid}})$ is already in permanent-collapse territory but experiences only gradual deterioration, making the collapse invisible on a standard fiscal-quarter horizon. The correct safety threshold is $\Delta F_{\mathrm{collapse}}$; $\Delta F_{\mathrm{rapid}}$ characterises collapse speed, not safety. \Halmos
\end{proof}


\subsection{Proof of Proposition 1 (Complementarity)}\label{sec:ec:proof4}
...
\textbf{Part 2: $F_{\max}(W)$ at the regime boundary.} $F_{\max}(W)$ is defined as the value of $F$ at which the surplus equilibrium of Lemma~1 ceases to exist, i.e., at which $S^* = 0$. From Lemma~1, at $S^* = 0$:
\[
...
The theoretical analysis uses the exact piecewise-smooth operators $[\cdot]^+ = \max(\cdot, 0)$. Table~\ref{tab:ec:smooth} verifies that replacing $[G]^+$ with a tanh-smoothed approximation $[G]^+_\varepsilon = G/2 + G\tanh(G/\varepsilon)/2$ across bandwidths $\varepsilon \in \{0.001, 0.005, 0.01, 0.05\}$ shifts $F_{\mathrm{crit}}$ by less than 1\% for practically relevant bandwidths ($\varepsilon \leq 0.01$), confirming that all three theorems are invariant to smoothing in this range.
...
Table~\ref{tab:ec:sensitivity} reports how each theorem's qualitative result responds to $\pm 50\%$ perturbations of each parameter from its baseline value. For each perturbation, we recompute the relevant threshold ($F_{\mathrm{crit}}$, $\partial\,\mathrm{CLV}^*/\partial f$, or $\Delta F_{\mathrm{collapse}}$) and record whether the theorem's qualitative prediction holds, weakens, strengthens, or disappears.
...
\textbf{Key observations.} The parameter $p$ is the most uniformly influential: reducing it weakens all three theorems simultaneously, confirming the workforce capacity multiplier as the model's structural driver. The parameter $b$ is critical for Theorem~1: at $b = 1$ the Trap disappears entirely, as linear demand allows the back-stage to absorb growth through Loop B1. The parameter $\sigma$ is critical for Theorem~3: at $\sigma = 1$, symmetric overload and recovery eliminate the bistability that produces the tipping point. No single perturbation simultaneously kills all three results.
...
\textbf{Demand super-linearity ($b = 1.3$).} \citet{bell2018inventory} and \citet{acimovic2015fulfillment} document fulfilment patterns consistent with error rates rising super-linearly with omnichannel combinations. We set $b = 1.3$ as an illustrative, conservative baseline motivated by this literature: a 10\% front-stage increase raises back-stage demand by approximately 13\%. The Trap requires only $b > 1$; the sensitivity range $b \in [1.0, 1.6]$ is in Table~\ref{tab:ec:sensitivity}.

\textbf{Workforce-technology complementarity ($p = 0.5$).} We set $p = 0.5$ as a moderate illustrative value: a burned-out workforce ($W = 0$) delivers 50\% of design capacity. The capital-skill complementarity estimates of \citet{krusell2000capital} support a mid-range starting point. The central testable prediction is the interaction coefficient in a store-level CLV regression on $K$, $W$, and $K \times W$, requiring instrumental variables to address simultaneity (see \S 6.2 of the main paper). All results hold for any $p \in (0,1)$.

\textbf{Overload-recovery asymmetry ($\sigma = 2.0$).} Set as a magnitude comparator consistent with \citeauthor{oliva2001cutting}'s (\citeyear{oliva2001cutting}) finding that workers are roughly twice as willing to cut corners as to work overtime. This is a comparator from a professional-service study, not a direct structural estimate for retail; the Job Demands-Resources (JD-R) literature \citep{bakker2007job} provides additional support. The Cliff disappears only at $\sigma = 1$; all theorems hold across $\sigma \in [1.0, 3.0]$ (Table~\ref{tab:ec:sensitivity}).

\subsection{Simulation Initial Conditions}

Table~\ref{tab:ec:simparams} reports the initial conditions and control parameters for each figure. All simulations use the baseline model parameters from Table~2 of the main paper. Initial conditions are chosen so that each simulation begins in the surplus regime ($\Keff > D$), representing a retailer with adequate but not excessive back-stage capacity.

\textbf{Basin-of-attraction map (referenced in \S5.2 of main paper).} We sweep $W_0 \in \{0.08, 0.10, \ldots, 0.80\}$ (37 points) and $F \in \{0.70, 0.75, \ldots, 1.95\}$ (26 points) on a grid, starting each trajectory at $K_0$ such that $G = 0$ and $T_0 = 0.50$. Terminal CLV $> 600$ marks convergence to the surplus attractor (green); CLV $< 600$ marks the low-trust attractor (red). The separatrix boundary is consistent with the analytic $\Delta F_{\mathrm{collapse}}$ threshold from Theorem~3. Convergence is validated by a full doubled-grid recomputation: the basin is re-run at $73{\times}51 = 3{,}723$ trajectories (step $\Delta W_0 = 0.01$, $\Delta F = 0.025$, half the original grid spacing in each dimension), and the separatrix $W_0$-position is compared at each common $F$-slice. The maximum shift across all $F$-slices is $0.0075$ $W_0$-units (fewer than 1 coarse grid step; tolerance $\Delta W < 0.04$), confirming that the boundary position is numerically stable. For the Burnout Cliff simulation (Figure~5), permanent collapse is defined as $T(t=500) < 0.15$ (surplus attractor: $T^* > 0.40$; low-trust: $T^* < 0.10$; confirmed stable at $t=500$ months).
...
We model CLV using the geometric-series structure of \citet{gupta2004valuing}: $\mathrm{CLV} = w(T) / (1 - \rho(T))$, where $\rho(T)$ is an annual retention probability and $w(T)$ an annualised per-customer margin. We adopt the specification $\rho(T) = 0.80 + 0.18T$ and $w(T) = 80(1 + 0.25\max(T-0.15, 0))$. The retention endpoints ($\rho(0) = 0.80$, $\rho(1) = 0.98$) match the 70--90\% annual range in \citet{gupta2004valuing}; \pounds 80 is an illustrative annual margin for UK omnichannel grocery. The model's analytical results require only that CLV is increasing in $T$, which holds for any $\rho'(T) > 0$, $w'(T) \geq 0$.

The kink in $w(T)$ at $T = 0.15$ does not affect any analytical result: Theorem~2 requires only $\partial\,\mathrm{CLV}/\partial T^* > 0$, which holds everywhere since $\partial\,\mathrm{CLV}/\partial T^* \geq w(T^*)\rho'(T^*)/(1-\rho(T^*))^2 > 0$ even at the kink. A softplus replacement produces CLV values within 0.5\% and qualitatively identical outputs.
