...
Differentiating: $\partial W^*/\partial f = \mu / (S^* + f + \mu)^2 > 0$, positive and decreasing in $f$ (diminishing returns).
...
\textbf{Step 3: Dynamics and timescales.} Under overload, the tipping threshold $W_{\mathrm{tip}} = f/(f+\mu+\sigma G_0^+)$ marks where depletion exceeds recovery. Two timescales govern speed: $\tau_{\mathrm{adapt}} = 1/r \approx 2.5$ mo and $\tau_{\mathrm{deplete}} = 1/(\mu+\sigma G_0^+) \in [1.5,4]$ mo. Near $\Delta F_{\mathrm{collapse}}$, convergence may require hundreds of months.
...
% Counter sync: preserve Proposition 4 numbering in restated theorem.
...
\begin{remark}[Concave Capacity Multiplier]
The constant cross-partial $\partial^2 \Keff / \partial W\,\partial K = p$ is an immediate consequence of the linear specification $h(W) = (1-p) + pW$. Under a concave alternative $h(W) = (1-p) + pW^\gamma$ ($\gamma < 1$), the cross-partial becomes $p\gamma W^{\gamma-1}$, which is state-dependent: complementarity is stronger when the workforce is depleted (low~$W$), precisely when it matters most operationally. The linear form therefore understates the complementarity effect under overload, making the main-paper result a conservative bound. Section~7.2 of the main paper discusses additional implications of concave~$h$.
\end{remark}


\subsection{Proof of Corollary 5 (Budget-Constrained Trap)}\label{sec:ec:proof5}

% Counter sync: preserve Corollary 5 numbering in restated theorem.
\setcounter{corollary}{4}
\begin{corollary}[Budget-Constrained Trap, restated]
Under a fixed budget $c_F F + c_f f \leq B$ with $B \geq B_{\min} - \varepsilon$ (where $B_{\min} = c_F \cdot F_{\mathrm{crit}}$), the CLV-maximising allocation includes $f^* > 0$: the all-$F$ corner is strictly dominated.
\end{corollary}

\begin{proof}
Let $B_{\min} \equiv c_F \cdot F_{\mathrm{crit}}$. \textit{Case 1} ($B \geq B_{\min}$): The all-$F$ corner sets $F \geq F_{\mathrm{crit}}$, so Theorem~1 gives $\partial\,\mathrm{CLV}/\partial F \leq 0$ while Theorem~2 gives $\partial\,\mathrm{CLV}^*/\partial f > 0$; reallocating the marginal dollar to $f$ yields a strict CLV gain.
\textit{Case 2} ($B < B_{\min}$): A surplus equilibrium exists. Theorem~2 guarantees $\partial\,\mathrm{CLV}^*/\partial f > 0$, so the all-$F$ corner forgoes a positive return. Near $B_{\min}$, per-dollar returns are continuous (IFT on $W^*(S)$, $T^*(S)$), and Case~1's strict dominance persists in a neighbourhood $B \in (B_{\min}{-}\varepsilon, B_{\min})$. For $B \in [B_{\min}/2, B_{\min}{-}\varepsilon)$, dominance holds computationally across $c_F/c_f \in [0.5, 5]$ at baseline values of all other parameters. \Halmos
\end{proof}
...
\bottomrule
\end{tabular}}
{The smoothing bandwidth $\varepsilon$ replaces exact $[\cdot]^+$ operators with $\tfrac{1}{2}(x + \sqrt{x^2 + \varepsilon^2})$. All three theorems are qualitatively invariant for $\varepsilon \leq 0.01$.}
\end{table}
...
\bottomrule
\end{tabular}}
{Each cell reports the qualitative effect of the perturbation on the theorem's central prediction. \emph{Holds}: qualitative sign unchanged relative to baseline. \emph{Weakens}: effect direction preserved but threshold shifts toward baseline (smaller safe zone or weaker buffer). \emph{Strengthens}: effect becomes more pronounced. \emph{Disappears}: result no longer obtains at the perturbation value. All perturbations are $\pm 50\%$ from the baseline in Table~2 of the main paper.}
\end{table}
...
All time units are months; rate parameters are in months$^{-1}$ with half-lives $\ln(2)/\theta$. The calibration follows \citet{homer1993partial} and \citet{barlas1996formal}; baseline values, timescales, kill conditions, and empirical anchors are reported in Table~2 of the main paper. Three parameters warrant additional discussion.
...
\textbf{Workforce-technology complementarity ($p = 0.5$).} A burned-out workforce ($W = 0$) delivers 50\% of design capacity; the value is illustrative and all results hold for any $p \in (0,1)$.

\textbf{Overload-recovery asymmetry ($\sigma = 2.0$).} Magnitude comparator from \citet{oliva2001cutting}; independently consistent with the loss-aversion ratio (${\approx}2.25$) from prospect theory \citep{tversky1992advances}. The Cliff disappears only at $\sigma = 1$ (Table~\ref{tab:ec:sensitivity}).
...
5 & $(K^*\!,W^*\!,T^*)$ & 0.80 & $0.05^{\ddagger}$ & 0.01--0.10 & 500 & 61-pt $\Delta F$ sweep \\
EC & eq.~IC at $F{=}0.85$ & 0.85 & 0.05 & 0.00--0.20 & 120 & Study~3: Thm~2 decomp.; $p{=}0$ counterfactual confirms structural-zero \\
\bottomrule
\end{tabular}}}
{$^{\dagger}$Balance manifold: $K_0 h(W_0)=(1{+}a{\cdot}0.5)F^b$, $T_0{=}0.50$; collapse criterion: $T(500){<}0.15$. $^{\ddagger}$Inherited from equilibrium.}
\end{table}
...
\subsection*{Event-Study Launch Identification}

The event study identifies the launch year as the fiscal year in which each firm's primary omnichannel fulfilment service (curbside pickup, click-and-collect, or same-day delivery) reached nationwide or near-nationwide availability. Launch years are identified from company press releases, investor presentations, and 10-K disclosures. Representative examples: Kroger ClickList (2016), Walmart Grocery Pickup (2017), Target Drive Up (2018), Albertsons Drive Up \& Go (2017), Costco same-day (2017). Amazon is excluded because its omnichannel transition was gradual. The complete 16-firm launch mapping is documented in the replication code (\texttt{09\_event\_study.py}).

\subsection*{State Minimum Wage Check}

A natural concern is that the county-wage buffer reflects state minimum wage policy rather than employer-level workforce investment. We test this by interacting the state minimum wage (standardised, time-varying, 2015--2024) with national e-commerce share in the county panel specification. The minimum wage interaction is not significant ($\hat{\beta}_1 = -0.0005$, $p = 0.22$; county and year FE, clustered at state, 53 clusters). In a horse-race specification including both the county-wage and state-MW interactions, the county-wage coefficient strengthens ($\hat{\beta}_1 = 0.0029$, $p = 0.020$) while the MW coefficient is marginally negative ($\hat{\beta}_1 = -0.0007$, $p = 0.063$). These results confirm that the Good Jobs Buffer operates through employer-level wage decisions above the mandated floor, consistent with the model's $f$ as a firm-level choice rather than a regulatory instrument. The MW change specification ($\Delta\mathrm{MW}$ since 2015 interacted with e-commerce) is similarly insignificant ($p = 0.23$).
...
The between-effects interaction is positive ($+$0.47; CR2: $p = 0.045$) but fails robustness (HC3: $p = 0.53$; Webb bootstrap: $p = 0.62$), reflecting $N_c = 11$ clusters \citep{cameron2008bootstrap, webb2023reworking}. Additional checks (currency-neutral controls, UK-only/Continental splits, leave-one-out with $K{\times}W \in [-0.04, +0.82]$, Imbens--Kolesar $\mathrm{df}_{\mathrm{eff}}{=}7.2$, $p{=}0.055$) are reported as exploratory.
}% end \small
